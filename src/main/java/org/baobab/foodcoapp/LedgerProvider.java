...
public class LedgerProvider extends ContentProvider {
    ...
    public boolean onCreate() {
        ...
        Cursor txns = db.getReadableDatabase().rawQuery("select * from transactions" +
                " join transaction_products on transactions._id = transaction_products.transaction_id" +
        ...
        while (txns.moveToNext()) {
            if (id != txns.getInt(0)) {
                if (0.01 < sum || sum < -0.01) {
                    System.out.println("TADA!!! sum is " + sum + msg);
                }
                sum = 0;
                msg = "";
                id = txns.getInt(0);
            ...
            float s = txns.getFloat(10) * txns.getFloat(11);
            msg += "\n    " + txns.getLong(6) + " " + txns.getFloat(10) + " x " + txns.getString(13) + " a " + txns.getFloat(11) + " = " + s;
            sum += s;
        }

        Cursor all = db.getReadableDatabase().rawQuery("select *" +
        ...
        while (all.moveToNext()) {
            if (!all.getString(8).equals("lager")
                    && !all.getString(8).equals("inventar")
                    && !all.getString(8).equals("kosten")
                    && !all.getString(8).equals("lager")
                    && !all.getString(8).equals("bank")
                    && !all.getString(8).equals("kasse")
                    && !all.getString(8).equals("forderungen")
                    && !all.getString(8).equals("einlagen")
                    && !all.getString(8).equals("beiträge")
                    && !all.getString(8).equals("verbindlichkeiten")
                    && !all.getString(8).equals("lager")) {
                //System.out.println("AHA! " + all.getString(8));
                float s = all.getFloat(10) * all.getFloat(11);
                //System.out.println("\n    " + all.getFloat(10) + " x " + all.getString(13) + " a " + all.getFloat(11) + " = " + s);
            }
        }
        all = db.getReadableDatabase().rawQuery("select " +
                "account_guid, SUM(transaction_products.quantity * transaction_products.price)" +
                " from transactions" +
                " join transaction_products on transactions._id = transaction_products.transaction_id" +
                " where transactions.status = 'final'" +
                " group by account_guid", null);
        float korns = 0;
        while (all.moveToNext()) {
            korns += all.getFloat(1);
            System.out.println(all.getString(0) + " : " + all.getFloat(1));
        }
        System.out.println("Guthaben " + korns);

        try {
            Cursor stock = query(Uri.parse("content://org.baobab.foodcoapp/accounts/lager/products?before=" +
                    BackupExport.df.parse("01/03/2016").getTime()), null, null, null, null);
            while (stock.moveToNext()) {
                System.out.println(" ~ " + stock.getString(7) + ": " + stock.getFloat(4) + " a " + stock.getFloat(5));
            }
        ...
        cv.put("title", "Korns");
        db.getWritableDatabase().update("transaction_products", cv, "_id = 2353", null);
        db.getWritableDatabase().update("transaction_products", cv, "_id = 2356", null);
        db.getWritableDatabase().update("transaction_products", cv, "_id = 2358", null);

        cv = new ContentValues();
        cv.put("price", 35.5059013366699);
        db.getWritableDatabase().update("transaction_products", cv, "_id = 1868", null);
        cv = new ContentValues();
        cv.put("price", 37.357753188521755);
        db.getWritableDatabase().update("transaction_products", cv, "_id = 1867", null);
        db.getWritableDatabase().delete("transaction_products", "_id = 1870", null);
        return false;
    }
            ...
            case PRODUCT_TRANSACTIONS:
                result = db.getReadableDatabase().rawQuery(
                        "SELECT transaction_products._id," +
                                " strftime('%d/%m/%Y',transactions.start/1000,'unixepoch') AS d," +
                                " SUM(transaction_products.quantity), transaction_products.unit," +
                                " ROUND(transaction_products.price, 2) AS p, transaction_products.title," +
                                " SUM(transaction_products.quantity * transaction_products.price), MAX(transactions.start)" +
                                "" +
                        ...
                        " GROUP BY d, transaction_products.title, p" +
                        " ORDER BY transactions.start", null);
                break;
